...
class SitesController extends Controller
{
    ...
    public function get(Request $request) {
        $sites=new Sites();
        if($request->search!='') {
            $sites=$sites->where('KuTitle','like','%'.$request->search.'%')->orWhere('EnTitle','like','%'.$request->search.'%')->orWhere('lat','like','%'.$request->search.'%')->orWhere('long','like','%'.$request->search.'%');
        }
