        ...
        <b-row>
            <b-col xl="10" lg="9" md="9" sm="12" cols="12" class="overflow-x:auto order-sm-2 order-md-2 order-2"
                ...
                <b-container class="mt-5">
                    ...
                    <!-- Main table element -->
                    <b-table show-empty small stacked="md" class="mt-3 rtl" :items="newses" :fields="fields" :busy="isBusy"
                        bordered>
                    ...
                    <b-modal id="infoModal" :content-class="{
                        ...
                        <div class="form-input">
                            <input type="text" autocomplete="off" required name="KuTitle" v-model="news.KuTitle" />
                        ...
                        <div class="form-input">
                            <input type="text" autocomplete="off" required name="EnTitle" v-model="news.EnTitle" />
                        ...
                        <div class="form-input">
                            <input type="text" autocomplete="off" required name="ArTitle" v-model="news.ArTitle" />
                        ...
                        <div class="form-input">
                            <textarea name="" id="" cols="30" rows="10" v-model="news.KuDesc">{{ $t("KuDesc") }}</textarea>
                            <span class="error text-danger" v-if="errors.KuDesc">{{ errors.KuDesc[0] }}</span>
                        </div>

                        <div class="form-input">
                            <textarea name="" id="" cols="30" rows="10" v-model="news.EnDesc">{{ $t("EnDesc") }}</textarea>
                            <span class="error text-danger" v-if="errors.EnDesc">{{ errors.EnDesc[0] }}</span>
                        </div>


                        <div class="form-input">
                            <textarea name="" id="" cols="30" rows="10" v-model="news.ArDesc">{{ $t("ArDesc") }}</textarea>
...
export default {
    ...
    methods: {
        edit(item, index) {
            ...
            this.news.KuTitle = item.KuTitle;
            this.news.EnTitle = item.EnTitle;
            this.news.ArTitle = item.ArTitle;
            this.news.KuDesc = item.KuDesc;
            this.news.EnDesc = item.EnDesc;
            this.news.ArDesc = item.ArDesc;

            this.news.imageUrl = '/images/' + item.image;
            this.$root.$emit("bv::show::modal", "infoModal");
        },
        choosedfile() {
            this.news.image = this.$refs.file.files[0];
            this.file_msg = this.$refs.file.files[0].name;
            this.news.imageUrl = URL.createObjectURL(this.news.image);
        },
        save() {
            ...
            formdata.append("KuTitle", this.news.KuTitle);
            formdata.append("KuDesc", this.news.KuDesc);
            formdata.append("ArTitle", this.news.ArTitle);
            formdata.append("ArDesc", this.news.ArDesc);
            formdata.append("EnTitle", this.news.EnTitle);
            formdata.append("EnDesc", this.news.EnDesc);
        ...
        get(page) {
            this.isBusy = true;
            axios
                .get("/api/news/get?page=" + page + "&search=" + this.search)
                .then((response) => {
